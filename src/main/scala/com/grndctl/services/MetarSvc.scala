...
class MetarSvc(implicit ec: ExecutionContext)
    extends InputStreamUnmarshaller[metar.Response](classOf[metar.Response])
    with LazyLogging {
  ...
  def getCurrentMetar(station: String): Future[Seq[METAR]] = {
    unmarshall(
      new URL(
        RequestUrl + MostRecentConstraint + StationString + station + HrsBefore + OneHr
      )
  ...
  def getMetars(station: String, hrsBefore: Double): Future[Seq[METAR]] = {
    unmarshall(
      new URL(RequestUrl + HrsBefore + hrsBefore + StationString + station)
    ).map(_.getData.getMETAR.asScala)
      .recover {
        case e: Exception =>
